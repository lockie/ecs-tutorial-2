...
case $1 in
    linux)
        do_build
        DEPLOY_GTK_VERSION=3 linuxdeploy --appimage-extract-and-run --executable=bin/ecs-tutorial-2 \
                    --custom-apprun=package/AppRun \
                    --icon-file=package/icon.png \
                    --desktop-file=package/ecs-tutorial-2.desktop \
                    --plugin gtk \
                    --appdir=appimage $(find bin -name "lib*" -printf "-l%p ")
        cp bin/ecs-tutorial-2 appimage/usr/bin
        cp -R Resources appimage/usr
        appimagetool --appimage-extract-and-run --comp zstd -g appimage "ecs-tutorial-2-${VERSION}.AppImage"
        ;;
    ...
    macos)
        ...
        xxd -ps -c1 bin/ecs-tutorial-2 | tr -d '\n' | sed "s/$replace_fr/$replace_to/" | fold -w 2 | xxd -r -p > "$contents/MacOS/ecs-tutorial-2"
        chmod +x "$contents/MacOS/ecs-tutorial-2"

        hdiutil create -quiet -srcfolder "$bundle" out.dmg
        # NOTE: ULMO = lzma compression = Catalina+ only
        hdiutil convert -quiet out.dmg -format ULMO -o "ecs-tutorial-2-${VERSION}.dmg"
        rm out.dmg
        ;;
